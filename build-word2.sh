...
# ==========================================================================
# IJRI — Word upload v2.
#   #1  any logged-in role (author/editor/chief/admin) can upload
#   #2  inline images preserved as embedded (base64) pictures
#   #3  title / abstract / body are auto-filled (no more blank blocks)
#   + live preview of the converted article before submitting
# Posts to /api/submissions with { title, authorNames, affiliation, sectionId,
# abstract, bodyHtml }. If your submissions route uses different field names,
# tell me and I'll match it.
# Run in repo:  bash build-word2.sh  ->  npm run build
# ==========================================================================
set -euo pipefail
echo "Word upload v2..."
...
mkdir -p "$BASE/submit/word" src/types
echo "  word page -> $BASE/submit/word"

# ---------------------------------------------------------------- mammoth types (browser build)
...
# ---------------------------------------------------------------- server page (gate + sections)
cat > "$BASE/submit/word/page.tsx" << 'IJRI_EOF'
...
# ---------------------------------------------------------------- client uploader
cat > "$BASE/submit/word/WordUpload.tsx" << 'IJRI_EOF'
"use client";
...
echo ""
echo "Word upload v2 written. Now run:  npm run build"
echo "NOTE: if embedded images convert but don't show on the PUBLISHED article,"
echo "      the sanitizer is stripping <img>/data: — send me src/lib/sanitize.ts."
